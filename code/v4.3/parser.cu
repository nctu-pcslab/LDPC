...
void CIRCUIT::Parser(char** argv){
    ...
    /*for(unsigned i=0; i<m; i++){
        int size = CheckNode[i]->BitNode_G.size();
        for(int j=1; j< NumOfCodeWord; j++){
            if(CheckNode[i+j*m]->BitNode_G.size() != size)
                cout<<size<<"!="<<CheckNode[i+j*m]->BitNode_G.size()<<endl;
        }
    }
    for(unsigned i=0; i<k; i++){
        int size = CheckNode[i]->BitNode_H.size();
        for(int j=1; j< NumOfCodeWord; j++){
            if(CheckNode[i+j*m]->BitNode_H.size() != size)
                cout<<size<<"!="<<CheckNode[i+j*m]->BitNode_H.size()<<endl;
        }
    }
    for(unsigned i=0; i<n; i++){
        int size = BitNode[i]->CheckNode_G.size();
        int size2 = BitNode[i]->CheckNode_H.size();
        for(int j=1; j< NumOfCodeWord; j++){
            if(BitNode[i+j*n]->CheckNode_G.size() != size)
                cout<<size<<"!="<<BitNode[i+j*n]->CheckNode_G.size()<<endl;
            if(BitNode[i+j*n]->CheckNode_H.size() != size2)
                cout<<size<<"!="<<BitNode[i+j*n]->CheckNode_H.size()<<endl;
        }
    }
